...
class BertForMaskedLM_sfmx(BertPreTrainedModel):
    def __init__(self, config, trees_path, device='cuda'):
        super().__init__(config)
        ...
        self.init_weights()
        self.cls.predictions.transform = None
        self.hsfmx = nn.Linear(config.hidden_size, 101)
    ...
    Returns:
        ...
        sequence_output = outputs[0]
        prediction_scores = self.hsfmx(sequence_output)
        ...
        if masked_lm_labels is not None:
            ...
            masked_lm_loss = loss_fct(prediction_scores.view(-1, 101), masked_lm_labels.view(-1))
        ...
        if lm_labels is not None:
            ...
            ltr_lm_loss = loss_fct(prediction_scores.view(-1, 101), lm_labels.view(-1))
