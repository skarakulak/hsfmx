...
def main(save_dir, transformers_cache_dir):
    ...
    if os.path.isfile(corr_path):
        ...
        with open(corr_path, 'wb') as handle:
            pickle.dump(cor_pd, handle, protocol=pickle.HIGHEST_PROTOCOL)
    ...
    for i in pbar:
        ...
        for j in range(n_partitions):
            ...
            gmm_clustering(
                    cor_pd.iloc[idx_dict_tree[j]].reset_index(drop=True),
...
if __name__ == '__main__':
    ...
    main(options.save_dir, options.transformers_cache_dir)
